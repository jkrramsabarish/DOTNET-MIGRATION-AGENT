...
- **Mode-aware required inputs:** in **Full Project** mode `build-result.json` is required; if it is missing there, the pipeline was interrupted — write a partial report with that notice. In **Single-File** and **Multi-File** modes the build/test steps are intentionally skipped, so `build-result.json`, `test-result.json`, `solution-map.json`, and `dependency-report.json` legitimately do NOT exist — generate the report from whatever is present and mark the skipped steps as "N/A for this mode", not as an interruption.
...
|---|---|---|
| `migrated-output/{repoName}/.migration/solution-map.json` | Step 1 agent | ✅ Full Project only (absent in Single/Multi-File) |
| `migrated-output/{repoName}/.migration/dependency-report.json` | Step 2 agent | ✅ Full Project only (absent in Single/Multi-File) |
| `migrated-output/{repoName}/.migration/compatibility-report.json` | Step 3 agent | ✅ All modes |
...
❌ Build still failed after {buildIterationsToGreen} build→fix iterations.
...
**Output preserved (rollbackOnFailure: false, default):** `migrated-output/{repoName}/` is kept so you can finish the remaining fixes. The original source was never modified. See the `// TODO [MIGRATION]` markers and the errors above.
{/if}
{if rollbackOnFailure == true}
**Rollback performed (rollbackOnFailure: true):** `migrated-output/{repoName}/` was deleted. The original source was never modified, so the workspace is back to its pre-migration state for this repo.
{/if}
{/if}
...
*Generated by .NET Migration Agent System v3.1.0 (output-isolation model)*
*{timestamp}*
```
...
| Build failed after fix loop, `rollbackOnFailure: false` (default) — output preserved | `❌ Build Failed — Output Preserved for Manual Fix` |
| Build failed after fix loop, `rollbackOnFailure: true` — output deleted | `❌ Failed — Rolled Back` |
| Build succeeded, no tests | `✅ Complete (No Tests)` |
| Single-File / Multi-File mode (no build/test step) | `✅ Complete (Code Migrated — build/test N/A for this mode)` |
...
| Failure | Action |
|---|---|
| `build-result.json` missing in **Full Project** mode | Write partial report with "Pipeline was interrupted" notice |
| `build-result.json` missing in **Single-File / Multi-File** mode | Expected — build step is skipped in these modes. Mark build/test sections "N/A for this mode" and generate the report normally |
...
*Agent Version: 3.1.0 | Read + Write (report only) | Pipeline Step: 7 of 7*