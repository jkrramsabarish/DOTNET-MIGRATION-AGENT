...
- Load ALL output JSON files from `.migration/` on invocation.
- Generate `migration-report.md` at the project root — not inside `.migration/`.
...
| Reads | All `.migration/*.json` files |
| Writes | `migration-report.md` at project root |
...
| `.migration/test-result.json` | Step 6 agent | Optional (may be absent if build failed) |

---

## OUTPUT

**Primary output:** `migration-report.md` at project root.
...
6. Delete `migration-backup/` once you have confirmed the migration is stable.
...
| Files Backed Up | {backupFileCount} |
| Backup Created At | {backupTimestamp} |

To roll back manually at any time:
```bash
# Restore all original files from backup
cp -r migration-backup/src ./src
cp migration-backup/global.json ./global.json
```

---

*Generated by .NET Migration Agent System v2.0.0*
*{timestamp}*
```
...
- Write to project root (same level as `.sln` file).
...
*Agent Version: 2.0.0 | Read + Write (report only) | Pipeline Step: 7 of 7*